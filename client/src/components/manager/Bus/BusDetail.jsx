...
const BusDetail = ({ busDetail, setBusDetail }) => {
    ...
    const handleSave = async () => {
        try {
            ...
            if (res?.success) {
                // Nếu driver là ID (string), thì dùng trực tiếp
                const driverId =
                    typeof busUpdate.driver === "string"
                        ? busUpdate.driver
                        : busUpdate.driver?._id;

                if (driverId) {
                    await UserService.updateDriverInfo(busUpdate.driver, {
                        assignedBus: busDetail?._id || busUpdate?._id,
                    });
                }
                ...
                if (busDetail.routeNumber) {
                    await RouteService.updateRoute({
                        ...
                        buses: (busDetail.buses || [])
                            .filter(
                                (bus) => bus.busNumber !== busDetail.busNumber
                            )
