...
const mockHistory = [
  ...
  // Thêm dữ liệu để thấy thanh cuộn
  {
    id: "h4",
    to: "Tất cả Tài xế",
...
const MessageTab = () => {
  ...
  // Cập nhật danh sách người nhận cụ thể khi vai trò thay đổi
  useEffect(() => {
  ...
  return (
    // Cấu trúc wrapper này được giữ nguyên từ StudentTab.jsx
    <div className={cx("tab-wrapper")}>
      {/* KHỐI BÊN TRÁI (LEFT-BLOCK) - HIỆN LÀ LỊCH SỬ TIN NHẮN (LIST) */}
      ...
      {/* KHỐI BÊN PHẢI (RIGHT-BLOCK) - HIỆN LÀ SOẠN TIN NHẮN (DETAIL/FORM) */}
          ...
          <form className={cx("composer-form")} onSubmit={handleSend}>
            ...
            <div className={cx("form-group")}>
              ...
              >
                ...
                <option value="bus_arrived">Xe buýt... đã đến điểm</option>
                <option value="general_announcement">Thông báo chung</option>
              </select>
            </div>

            {/* --- Chọn vai trò muốn gửi --- */}
            <div className={cx("form-group")}>
              <label htmlFor="targetRole">Gửi đến (Vai trò)</label>
              <select
                id="targetRole"
                value={targetRole}
            ...
            {/* --- Chọn người nhận cụ thể (nếu áp dụng) --- */}
            ...
            {/* --- Nội dung tin nhắn --- */}
            <div className={cx("form-group")}>
              <label htmlFor="customMessage">Nội dung tin nhắn</label>
              <textarea
                id="customMessage"
                value={customMessage}
